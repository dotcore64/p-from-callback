...
describe("p-from-callback", () => {
  it("should resolve with 1 argument", () =>
    expect(fromCallback((callback) => callback(undefined, "foo"))).to.become(
      "foo",
    ));

  it("should resolve with 1 argument when callback has multiple", () =>
    expect(
  ...
  it("should resolve to a single element array", () =>
    expect(
      fromCallback((callback) => callback(undefined, "foo"), true),
    ).to.become(["foo"]));

  it("should resolve to undefined", () =>
    expect(fromCallback((callback) => callback())).to.become());

  it("should resolve to an empty array", () =>
    expect(fromCallback((callback) => callback(), true)).to.become([]));
