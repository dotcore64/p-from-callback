...
expectAssignable<Promise<string>>(
  fromCallback((cb) => cb(undefined, "foo", "bar")),
);
...
expectAssignable<string>(
  await fromCallback((cb) => cb(undefined, "foo", "bar")),
);
expectAssignable<string[]>(
  await fromCallback((cb) => cb(undefined, "foo"), true),
);
// @ts-expect-error
...
expectAssignable<string[]>(
  await fromCallback((cb) => cb(undefined, "foo", "bar"), true),
);
// @ts-expect-error
...
let foo: boolean;
expectAssignable<string>(
  await fromCallback((cb) => cb(undefined, "foo", "bar"), foo),
);
expectAssignable<string[]>(
  await fromCallback((cb) => cb(undefined, "foo", "bar"), foo),
);
...
fromCallback((cb) => cb(undefined, "foo", "bar"), foo) as Promise<number[]>;
// @ts-expect-error
fromCallback((cb) => cb(undefined, "foo", "bar"), foo) as Promise<
  number | number[]
>;
